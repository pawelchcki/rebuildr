...
from rebuildr.descriptor import Descriptor, Dockerfile, EnvInput, FileInput, GlobInput, TagTarget
...
class StableDescriptor:
    ...
    def _make_stable_files(files, absolute_path: Path) -> list[StableFileInput]:
        ...
        for file_dep in files:
            if isinstance(file_dep, FileInput):
                stable_files.append(StableFileInput(path=file_dep.path, absolute_path=absolute_path / file_dep.path))
            elif isinstance(file_dep, Dockerfile):
                stable_files.append(StableFileInput(path=file_dep.path, absolute_path=absolute_path / file_dep.path))
            elif isinstance(file_dep, GlobInput):
                glob_root = absolute_path if file_dep.root_dir is None else absolute_path / file_dep.root_dir
                for path in glob.glob(file_dep.pattern, root_dir=glob_root, recursive=True, include_hidden=True):
                    path = PurePath(path)
                    ...
                    if absolute_path.is_file():
                        stable_files.append(StableFileInput(path=path, absolute_path=absolute_path))
            else:
    ...
    def from_descriptor(descriptor: Descriptor, absolute_path: Path) -> Self:
        ...
        file_deps = StableDescriptor._make_stable_files(descriptor.inputs.files, absolute_path)

        env_deps = [StableEnvInput(key=dep.key, default=dep.default) for dep in descriptor.inputs.builders if isinstance(dep, EnvInput)]
        builder_deps = StableDescriptor._make_stable_files([ dep for dep in descriptor.inputs.builders if not isinstance(dep, EnvInput)], absolute_path)
